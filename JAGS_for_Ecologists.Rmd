...
```
I have also added a line of code to compute the total population size as a derived variable, $N=n_1 + N_0$. Note that this solution only works when the population size is large. There is another solution that works in all cases, but it is more technical. This code is provided in the file `Examples/Example_5/example_5a.jags`.

##### Example 5: Two-Sample Experiment
Suppose that a two-sample experiment is conducted and that 30 individuals are captured in the first sample ($n_1$), 37 unmarked individuals are captured in the second sample ($n_2$), and that of the 30 captured in the first sample 14 are also captured in the second sample ($m$). The LP estimate for the population size given these values is 109 (after rounding) and the Chapman estimate is 106.

The model can be run in JAGS with the following code:
```{r echo=TRUE, eval=FALSE}
## Construct JAGS data
jags_data_5a <- list(n1 = 30, n2 = 37, m = 14, pN0 = rep(1,1000))
...
```
You should find that the posterior mean for $N_0$ is about 117 and that the 95% credible interval extends from about 82 to 176. Again, your exact results may differ somewhat because of random variations in the sampling. 
...
Let $n$ denote the number of individuals captured at least once during the experiment. The data component of this model can be written as:
```{r eval=FALSE,echo=TRUE}
# Data distribution
  n ~ dbinom(p_ever,N)
  ...
  p_ever <- 1 - (1-p)^K
```
The same prior distributions can be applied to this model as in Example 5, and so we can write the full model as
...
```
The code is also provided in the file `Examples/Example_6/example_6a.jags`.

###### Example 6: More than two samples

As an example we will analyse a simulated data set, so we know the true population size. I generated the data with 5 capture occasions and a true population size of 123. A total of 105 individuals were captured at least once and the capture histories for the first three individuals look like this:
```{r echo = FALSE, eval=TRUE}
## Load data
data6 <- readRDS("Examples/Example_6/example_6_data.rds")

head(data6,3)
```
The full data is in the file `Examples/Example_6/example_6_data.rds`.

The following code will fit the model and generate summary statistics. Note that I have set the values of $n$, the number of individuals observed, and $K$, the number of capture histories, manually, but you could modify the code to compute these from the data. Once again, we need to set the initial value for $N$ to ensure the code will run. 
```{r, echo=TRUE,eval=FALSE}
## Load data
data6 <- readRDS("Examples/Example_6/example_6_data.rds")

## Construct JAGS data
jags_data_6 <- list(n = 105, K = 5, caps = data6, pN=rep(1,1000))

## Set initial values
jags_inits_6 <- list(N = 106)

## Compile model
jags_model_6a <- jags.model("Examples/Example_6/example_6a.jags", jags_data_6, inits = jags_inits_6)
...
```
You should find that the posterior mean of $N$ is about 118, which is very close to the truth, and that the 95% credible interval extends from about 110 to 130. 

Here is the code to create the visualization of the summary statistics using a caterpillar plot:
```{r echo = TRUE, eval = FALSE}
## Create ggs object
ggs_6a <- ggs(jags_samples_6a)

## Draw caterpillar plot for N
ggs_caterpillar(ggs_6a, "N")

## Draw caterpillar plot for p
ggs_caterpillar(ggs_6a, "p")
```

#### More than Two Samples -- Time-Varying $p$

There are many reasons that the capture probability might change across the capture occasions. Perhaps the capture of individuals depends on weather conditions. In this case, we might want the model to allow the capture probability to change over time. This is implemented with the following code:
```{r echo=TRUE, eval=FALSE}
model{
  # Data distribution
  n ~ dbinom(p_ever,N)
...
```
This looks very similar to what we had before, with a few minor changes. First, I have replaced the constant `p` with the occasion specific `p[k]` in the distribution of the capture histories. Second, I have assigned each of the new capture probabilities a separate uniform prior distribution using a loop over `k`. Third, I have modified the calculation of the probability of being captured at least once, `p_ever`, to account for the variation over time. 

#### Example 6 (continued)
```{r, echo=TRUE,eval=FALSE}
## Compile model
jags_model_6b <- jags.model("Examples/Example_6/example_6b.jags", jags_data_6, inits = jags_inits_6)
...
```
The output from this model will now display summary statistics for the capture probabilities on each of the 5 occasions. The posterior summary statistics can be visualized with the following code:
```{r echo=TRUE, eval=FALSE}
## Create ggs object
...
##### Example 6 (continued)
The following code implements the closed population mark-recapture model with the hierarichal model for $p$:
```{r echo=TRUE, eval = FALSE}
model{
  # Data distribution
  n ~ dbinom(p_ever,N)
...
```
Here I have replaced the uniform prior distribution for `p[k]` with the beta distribution with parameters `alpha` and `beta`. I have then assigned `alpha` and `beta` their own prior distributions. In this case, I have assigned both parameters to have gamma prior distributions. I won't go into specifics, but this is a common choice of prior for parameters that can be any positive number. 

The following code will run the model in `R`:
```{r, echo=TRUE,eval=FALSE}
## Compile model
jags_model_6c <- jags.model("Examples/Example_6/example_6c.jags", jags_data_6, inits = jags_inits_6)

## Generate samples
jags_samples_6c <- coda.samples(jags_model_6c, c("alpha","beta","N"), n.iter = 1000)

## Compute summary statistics
summary(jags_samples_6c)
```
Note that this time I monitored not the individual capture probabilities but the parameters `alpha` and `beta`. This allows us to describe the distribution of the capture probabilities over all occasions instead of commenting on the value of the capture probability for a specific occasion. You should find that the estimates of these parameters (the posterior means) are about 5 and 9. Given these values, we could say that the mean capture probability is $5/(5 + 9)=.36$. Using the function `qbeta()` in `R` we could compute the quantiles of the distribution:
```{r, eval = TRUE, echo = TRUE}
## Compute the 2.5% and 97.5%-iles of the distribution of p
qbeta(c(.025,.975), 5, 9)
```
If we were to collect another sample then we would guess that the capture probability would be about .36 and we could be 95% certain that it would be between about .14 and .61. You can replace the values 5 and 9 with the values from your summary to obtain specific results. 
...
##### Example 7: Dippers
```{r}
load("Examples/Example_7/dipper.rda")

```
As an example, we will look at a data on the European dipper collected in France and originally described by Lebreton et al. (1992). The data contain records on the capture of 294 dippers over 7 occasions. To start, we will look at the data only from the individuals that were captured on the first occasion. We will then extend the model to account for all of the individuals.
...
We might draw the DAG for the CJS model like this:

![DAG for Two-Sample $t$-test](Figures/cjs_dag.png){width=500px}
...
$$
The problem is that we can't always know when an individual is alive. If it is last seen on occasion 2, or example, then it might have dead between occasion 2 and occasion 3 or it might have lived from occasion 2 to occasion 3, not been captured on occasion 3, and then died between occasion 3 and occasion 4, or ... . Here, the variables $Z_{i,t}$ represent latent variables because they cannot always be observed. Even though we can't alwys observe these variables, we will include them in our model to because this makes the definition of the data distribution much, much simpler.

##### Example 7 (continued): Dippers

The following code implements the CJS model for the case in which all individuals are captured and marked on the first occasion. The code is included in the file `Examples/Example_7/example_7a.jags`. The first block of code implements the relationships between being alive on one occasion and both capture on the same occasion and remaining alive on the next occasion. The second block of code defines the prior distributions. I have again employed hierarchical distributions for the survival probability and the capture probability. Note that the capture probability is only included for occasions 2 through 7. This is because information about the capture probability comes from recapturing indivdiuals that were marked on previous occasions. The data contain no information about the capture probability on the first occasion since there are no marked individuals before this point.
```{r echo = TRUE, eval = FALSE}
model{
  ## Data distribution
  for(i in 1:n){
  ...
  ## Prior distributions
  for(t in 1:6){
    phi[t] ~ dbeta(alpha.phi, beta.phi)
  }

  alpha.phi ~ dgamma(.1,.1)
  beta.phi ~ dgamma(.1,.1)
  
  for(t in 2:7){
    p[t] ~ dbeta(alpha.p, beta.p)
  }
...
Now you can load the dipper data and run the model:
```{r,echo=TRUE, eval = FALSE}
## Load data
dipper1 <- read_csv("Examples/Example_7/dipper1.csv")

## Construct JAGS data
jags_data_7a <- list(n = 22, 
                    Y = as.matrix(dipper1[,3:9]))

## Set initial values
jags_inits_7a <- list(Z=cbind(NA,matrix(1, nrow = 22, ncol = 6)))
...
ggs_caterpillar(ggs_7a, "phi", sort = FALSE)

ggs_caterpillar(ggs_7a, "p\\[", sort = FALSE)
...
```{r echo = TRUE, eval = FALSE}
## Load data
dipper2 <- read_csv("Examples/Example_7/dipper2.csv")

## Construct JAGS data
Y <- as.matrix(dipper2[,3:9])
a <- apply(Y, 1, function(c) min(which(c == 1)))

jags_data_7b <- list(n = 254, Y = Y, a = a)

## Set initial values
Z <- t(sapply(a, function(a) c(rep(NA,a), rep(1,7-a))))

jags_inits_7b <- list(Z=Z)

## Compile model
jags_model_7b <- jags.model("Examples/Example_7/example_7b.jags", jags_data_7b, inits = jags_inits_7b)

## Generate samples
jags_samples_7b <- coda.samples(jags_model_7b, c("p","phi"), n.iter = 1000)
...
ggs_caterpillar(ggs_7b, "phi", sort = FALSE)

ggs_caterpillar(ggs_7b, "p\\[", sort = FALSE)
